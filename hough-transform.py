...
def line_detection_non_vectorized(image, edge_image, num_rhos=180, num_thetas=180, t_count=220):
    edge_height, edge_width = edge_image.shape[:2]  # height and whdth
    edge_height_half, edge_width_half = edge_height / 2, edge_width / 2  # half
    #
    # diagonal hypothenuse
    d = np.sqrt(np.square(edge_height) + np.square(edge_width))
    dtheta = 180 / num_thetas  # 0 - 180
    drho = (2 * d) / num_rhos
    #
    thetas = np.arange(0, 180, step=dtheta)  # 180 values
    rhos = np.arange(-d, d, step=drho)  # accuracy
    #
    cos_thetas = np.cos(np.deg2rad(thetas))  # thetas cos acos+bsin
    sin_thetas = np.sin(np.deg2rad(thetas))  # thetas sin
    #
    accumulator = np.zeros((len(rhos), len(rhos)))  # intersect
    #
...
if __name__ == "__main__":
    ...
    edge_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # gray scale
    edge_image = cv2.GaussianBlur(edge_image, (3, 3), 1)  # downscale
    edge_image = cv2.Canny(edge_image, 100, 200)  # white edges
